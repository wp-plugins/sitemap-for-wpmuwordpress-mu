...
<?php 
		...
		foreach ($blogs as $blog) {
			...
			echo "<url><loc>http://". $blog->domain. $blog->path. "</loc>";
			echo "<lastmod>". mysql2date('Y-m-d\TH:i:s+00:00', $blog->last_updated, false). "</lastmod>";
			...
			if ($post_ids) {
				...
				while ( $next_posts = array_splice($post_ids, 0, 20) ) {
					...
					$query_jiang = "SELECT * FROM $blogPostsTable WHERE post_status = 'publish' 
						AND post_type = 'post' ORDER BY post_date_gmt DESC";
					$posts = $wpdb->get_results($query_jiang);
					foreach ($posts as $post) {
						setup_postdata($post); 
?>
	<url><loc><?php echo get_blog_permalink($blog->blog_id, $post->ID); ?></loc>
